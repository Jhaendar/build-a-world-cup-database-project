...
echo "$($PSQL "SELECT name FROM (SELECT w.name FROM games INNER JOIN teams w ON games.winner_id = w.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT o.name FROM games INNER JOIN teams o ON games.opponent_id = o.team_id WHERE year=2014 AND round='Eighth-Final') eight_final")"
